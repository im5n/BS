...
echo "Broker is deprecated, please use QS instead."
...
echo "- Other temporary bypasses, check the \"Avoiding accidental re-enrollment\" for more info."
...
if [ -z "$USE_KERN" ]; then
	...
	[ -f "$KERN_PAYLOAD" ] || fail "Required payload '$KERN_PAYLOAD' not found! Is this image built with the correct payload for Broker?"
	[ -f "$ROOT_PAYLOAD" ] || fail "Required payload '$ROOT_PAYLOAD' not found! Is this image built with the correct payload for Broker?"
